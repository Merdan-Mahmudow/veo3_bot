...
@router.get("/{chat_id}")
async def get_partner_by_chat_id(chat_id: str, session: AsyncSession = Depends(get_session)):
...
async def get_partner_stats(partner_id: uuid.UUID, session: AsyncSession = Depends(get_session)):
    # Count registrations
...
async def create_referral_link(partner_id: uuid.UUID, percentage: int, session: AsyncSession = Depends(get_session)):
...
async def request_payout(partner_id: uuid.UUID, amount: float, session: AsyncSession = Depends(get_session)):
